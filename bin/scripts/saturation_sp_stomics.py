...
def fraction_reads(outdir,totalreads:int):
    ...
    stats_df.to_csv(os.path.join(outdir,"%s_Saturation.tsv"%name),sep='\t')
...
def plot_saturation():
    for_plot = pd.read_table(os.path.join(outdir,"%s_Saturation.tsv"%name),sep='\t')
